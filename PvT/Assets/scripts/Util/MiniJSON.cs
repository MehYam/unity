...
public static class MJSON
{
    ...
    public static void merge(this Hashtable thisObj, Hashtable sourceJson)
    {
        foreach(DictionaryEntry entry in sourceJson)
        {
            if (entry.Value is Hashtable)
            {
                // recurse to handle nested JSON elements
                var existing = thisObj[entry.Key];
                if (existing is Hashtable)
                {
                    merge((Hashtable)existing, (Hashtable)entry.Value);
                }
                else
                {
                    thisObj[entry.Key] = entry.Value;
                }
            }
            else
            {
    ...
    // lets you do things like .getNode(node, "jsonObject.child.grandchild")
    public static object getNode(object parentNode, string childPath)
    {
        try
        {
            object retval = parentNode;
            char[] period = { '.' };
            foreach (var key in childPath.Split(period))
            {
                retval = ((Hashtable)(retval))[key];
            }
            return retval;
        }
